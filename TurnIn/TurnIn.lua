...
function TI_VarInit()
	...
	if not TI_status.options or getn(TI_status.options) == 0 then
		TI_OptionsInit()
	end
...
function TI_Switch(state)
	if state == true then
		...
		TI_LoadEvents()
		TI_message("Turn In "..TI_ON)
	elseif state == false then
		TI_ResetPointers()
		TI_status.state = false
		TI_UnloadEvents()
		TI_message("Turn In "..TI_OFF)
	elseif state == "toggle" then
		TI_Switch(not TI_status.state)
	end
...
function TI_SlashCmdHandler(cmd)
	...
	local commands = {
		...
		status = function()
			TI_message("Turn In "..(TI_status.state and TI_ON or TI_OFF))
		end,
		config = function()
		...
		debug = function()
			if TI_status.debugstate then
				...
				TI_message("debug mode "..TI_OFF)
			else
				TI_status.debugstate = true
				TI_message("debug mode "..TI_ON)
			end
		end
	}
	...
	if commands[cmdlist[1]] then
		...
		TI_message(TI_CMD_HELP, "--------------", TI_CMD_1, TI_CMD_2, TI_CMD_3, TI_CMD_4, TI_CMD_5)
	end
...
function TI_IsNPCOn(npcname, type)
	...
	if not TI_NPCDB[npcname] and TI_status.usedefault == true and opton == true then
		TI_debug("case 1")
		return true
	elseif TI_NPCDB[npcname] and TI_NPCDB[npcname].state then
		TI_debug("case 2")
		return true
	elseif TI_NPCDB[npcname] and TI_status.usedefault == true and not TI_NPCDB[npcname].state and opton == true then
		TI_debug("case 4")
		return true
	else
		TI_debug("case 3")
...
function TI_CompleteQuest()
	if IsQuestCompletable() then
		TI_debug("quest is completable, completing")
		CompleteQuest()
		TI_ResetPointers()
	else
...
function TI_HandleGossipWindow(gorq)
	...
	local TotalOptions = getn(AvailableQuests)+getn(ActiveQuests)+getn(GossipOptions)
	...
	for i,current in ipairs(TI_status.options) do
		if current.state then
			if current.type == "availquest" and getn(AvailableQuests) > 0 and TI_availnumber <= getn(AvailableQuests) then
				TI_availnumber = TI_availnumber + 1
				SAvQ(TI_availnumber-1)
				TI_debug("Selecting Available Quest ".. TI_availnumber-1)
				return
			elseif current.type == "activequest" and getn(ActiveQuests) > 0 and TI_activenumber <= getn(ActiveQuests) then
				TI_activenumber = TI_activenumber + 1
				SAcQ(TI_activenumber-1)
				TI_debug("Selecting Active Quest ".. TI_activenumber-1)
				return
			elseif getn(GossipOptions) > 0 then
...
function TI_AddNPCToList(OptList, npcname, confirminquestion)	 
	...
	if getn(OptList) > 0 then
		if not TI_NPCDB[npcname] then
...
function TI_AddNPCToTempList(name, list)
	...
	if getn(TI_TempNPCList) > TI_TempNPCListMaxSize then

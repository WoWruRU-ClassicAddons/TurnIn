function TI_PopulateOptions(arg)
	...
	if TI_status and TI_status.options then
		...
		for i,v in ipairs(CurrOpts) do
			...
			local iconpath
			if not v.name then TI_debug("oh noes") end
...
function TI_PopulateOptions(arg)
	...
	if TI_status and TI_status.options then
		for i,v in ipairs(TI_status.options) do
			...
			local iconpath
			if not v.name then TI_debug("oh noes") end
			if not getglobal(entry .. "_Text") then TI_debug("wtf "..entry .. "_Text") end
...
function TI_StatusIndicatorUpdate()
	if TI_status then
		if TI_status.state then
			TI_StatusIndicator_Status:SetText(TI_ON)
		else
			TI_StatusIndicator_Status:SetText(TI_OFF)
		end
...
function TI_StatusIndicator_CheckFn(self)
	TI_Switch(self:GetChecked() and true or false)
end
...
function TI_NPCListCheckboxTooltip(self)
	if self:GetChecked() then
		TI_TooltipMessage(self, TI_NPC_SPECIFIC_SETTINGS)
	else
		TI_TooltipMessage(self, TI_NPC_DEFAULT_SETTINGS)
	end
end

function TI_OptionListCheckboxTooltip(self)
	if self:GetChecked() then
		TI_TooltipMessage(self, TI_OPTION_ENABLED)
	else
		TI_TooltipMessage(self, TI_OPTION_DISABLED)
	end
end
